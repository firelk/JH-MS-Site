        ...
        if(window.FileReader) {
            ...
            oReader.onload = function(e) {
                ...
                image.onload = function() {
                    ...
                    // uploadImg('Photo_A', 'zsta', "/v1/interface/img-upload2",);
...
function postImgAndroid(base64, imgId, inputId) {
    var uri = "/v1/interface/img-upload2";
    var pic = $('#' + imgId);
    var ipt = $('#' + inputId + '-src');
...
function postImgIphone(base64, imgId, inputId) {
    ...
    // fd.append("file", file); // 添加文件到表单
    // xhr.upload.addEventListener("progress", function(evt) {
    //     pic.attr('src', "/view/images/lodding.gif");
    // }, false);
    // xhr.open("POST", uri + "?img=" + pic.attr('src'), true);
    // xhr.send(fd);
