$(function() {
    ...
    $('.pass-submit').click(function(event) {
        ...
        if (p1.length == 6) {
            //提交
            // CommonUtil.redirectUrl('pay/pay_code.html?v_'+ (new Date().getTime()));
            // 获取会员额度
            $.ajax({
                    url: window._appPath + 'transpassword/valid.do',
                    type: 'post',
                    dataType: 'json',
                    data: {
                ...
                .done(function(data) {
                    ...
                    var json = data
                    console.log(json, '支付密码')
                    if (data.code == 0) {
                        $(".box-6-1").removeClass('hide')
                        $(".box-6").addClass('hide')
                        $(".btn").addClass('hide')
                        // 二维码
                        $("#code").qrcode({
                            ...
                            width: 200, //宽度 
                            height: 200, //高度 
                            text: data.data.paycode //任意内容 
                        });

                      // 条形码
                      JsBarcode("#barcode1", data.data.paycode,{
                        displayValue: false
                      });
                    } else {
                        $.alert(data.message)
            ...
            $('#payPassword_container').append('<span id="payPassword_rsainput-error" class="help-block"><i class="icon icon-attention icon-lg"></i>&nbsp;密码不够6位</span>')
